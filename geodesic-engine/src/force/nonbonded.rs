use geodesic_core::{AtomData, SimState};
...
///
/// Takes `pair_i`/`pair_j` as plain slices (rather than a whole
/// `&NeighborList`, whose `ref_{x,y,z}`/`r_skin` this function has no use
/// for) so cpu_backend.rs can hand each thread its own contiguous strip of
/// the flat pair list for the SAD.md §7.2 static decomposition, with no
/// intermediate copy.
///
...
pub fn compute_pair_forces(
    ...
    for (&i, &j) in pair_i.iter().zip(pair_j.iter()) {
